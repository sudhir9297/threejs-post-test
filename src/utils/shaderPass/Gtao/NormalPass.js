import { ShaderReplacement, setMaterialDefine } from "./ShaderReplacement";
...
export class NormalPass extends ShaderReplacement {
  ...
  updateUniforms(object, material, target) {
    ...
    target.setDefine(
      "USE_NORMALMAP",
      this.useNormalMaps && target.uniforms.normalMap.value ? "" : undefined
    );
    target.setDefine(
      "TANGENTSPACE_NORMALMAP",
      this.useNormalMaps && target.uniforms.normalMap.value ? "" : undefined
    ...
    target.setDefine(
      "ALPHATEST",
      target.uniforms.alphaTest.value
        ? target.uniforms.alphaTest.value
        : undefined
    );
    ...
    target.setDefine(
      "USE_ALPHAMAP",
      target.defines.ALPHATEST === 0 || !target.uniforms.alphaMap.value
        ? undefined
        : ""
    );
    ...
    target.setDefine(
      "USE_MAP",
      target.defines.ALPHATEST === 0 || !target.uniforms.map.value
        ? undefined
        : ""
    );
    ...
    target.setDefine(
      "USE_UV",
      "USE_ALPHAMAP" in target.defines || "USE_MAP" in target.defines
        ? ""
        : undefined
    );
  }
