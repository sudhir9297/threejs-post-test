...
import { PackedNormalDisplayShader } from "./DebugShaders2";
import { GTAOShader } from "./GTAOShader";
import { CompositeShader } from "./CompositeShader";
import { RendererState } from "./RendererState";
import { BlueNoiseGenerator } from "./BlueNoiseGenerator";
...
export class GTAOPass extends Pass {
  constructor(scene, camera) {
    ...
    this._gtaoBuffer = new WebGLRenderTarget(1, 1, {
      // minFilter: NearestFilter,
      // magFilter: NearestFilter,
      format: RGBAFormat,
    });

    this._depthBuffer = new WebGLRenderTarget(1, 1, {
      minFilter: NearestFilter,
      magFilter: NearestFilter,
      format: RGBFormat,
      type: HalfFloatType,
    ...
    this._normalBuffer = new WebGLRenderTarget(1, 1, {
      minFilter: NearestFilter,
      magFilter: NearestFilter,
      format: RGBAFormat,
    });
    ...
    this.debugPackedQuad = new FullScreenQuad(
      new ShaderMaterial(PackedNormalDisplayShader)
    );
    this.debugDepthQuad = new FullScreenQuad(
      new ShaderMaterial(LinearDepthDisplayShader)
    );
    this.compositeQuad = new FullScreenQuad(
      new ShaderMaterial(CompositeShader)
    );
  ...
  render(renderer, writeBuffer, readBuffer) {
    ...
    if (
      Math.pow(this.falloffStart, 2.0).toFixed(16) !==
        gtaoMaterial.defines.FALLOFF_START2 ||
      Math.pow(this.falloffEnd, 2.0).toFixed(16) !==
        gtaoMaterial.defines.FALLOFF_END2 ||
      this.enableFalloff !== Boolean(gtaoMaterial.defines.ENABLE_FALLOFF)
    ) {
      gtaoMaterial.defines.FALLOFF_START2 = Math.pow(
        this.falloffStart,
        2.0
      ).toFixed(16);
      gtaoMaterial.defines.FALLOFF_END2 = Math.pow(
        this.falloffEnd,
        2.0
      ).toFixed(16);
    ...
    if (
      (this.lightBounceIntensity !== 0.0) !==
      Boolean(gtaoMaterial.defines.ENABLE_COLOR_BOUNCE)
    ) {
      gtaoMaterial.defines.ENABLE_COLOR_BOUNCE =
        this.lightBounceIntensity !== 0.0 ? 1 : 0;
      gtaoMaterial.needsUpdate = true;
    }
    ...
    gtaoMaterial.uniforms.lightBounceIntensity.value =
      this.lightBounceIntensity;
    ...
    compositeMaterial.uniforms.aoSize.value.set(
      gtaoBuffer.width,
      gtaoBuffer.height
    );
    compositeMaterial.uniforms.fullSize.value.set(
      readBuffer.width,
      readBuffer.height
    );
